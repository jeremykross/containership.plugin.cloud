...
module.exports = {
        ...
        callback: function(options) {
            ...
            function authenticate(auth, method, callback) {
                ...
                const request_options = {
                    url: `${coreConfig.auth_url}/v1/authenticate/${method}/authorization`,
                    method: 'POST'
                };
    ...
    'list-orgs': {
        ...
        callback: function() {
            ...
            let request_options = {
                url: [
                    coreConfig.config.cloud_api_url,
                    'v2',
                    'account'
                ].join('/'),
                method: 'GET',
    ...
    'use-cluster': {
        ...
        callback: function(options) {
            config.load();
            coreConfig.load();
            config.config['api-url'] = coreConfig.cloud_api_url;
...
function makeConfig(jsonFile) {
    return {
        load: function() {
            try{
                this.config = JSON.parse(fs.readFileSync([process.env['HOME'], '.containership', jsonFile].join('/')));
            } catch(e) {
        ...
        set: function(new_config) {
            let config;
            try{
                config = JSON.parse(fs.readFileSync([process.env['HOME'], '.containership', jsonFile].join('/')));
            } catch(e) {
                config = {};
            }

            try{
                fs.writeFileSync([process.env['HOME'], '.containership', jsonFile].join('/'), JSON.stringify(new_config));
                this.config = config;
            } catch(e) {
...
let config =  makeConfig('cli.json');
let coreConfig = makeConfig('cloud.json');
