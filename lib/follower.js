...
module.exports = {
    ...
    initialize: function(core, config) {
        ...
        core.scheduler.follower.container.add_pre_pull_middleware('docker', 'authentication', function(options, fn) {
            ...
            let request_options = {
                url: [config.cloud_api_url, 'v2', 'organizations', config.organization, 'registries'].join('/'),
                method: 'GET',
                headers: {
        ...
        core.scheduler.follower.container.add_pre_start_middleware('docker', 'csc_env', function(container_options, fn) {
            ...
            let options = {
                url: [config.cloud_api_url, 'v2', 'organizations', config.organization, 'clusters', core.cluster_id].join('/'),
                method: 'GET',
                headers: {
                    Authorization: ['Bearer', config.api_key].join(' ')
                },
        ...
        core.scheduler.follower.container.add_pre_start_middleware('docker', 'containership-cloud', function(container_options, fn) {
            if(_.has(container_options.env_vars, 'CSC_BACKUP_ID') && !_.isEmpty(container_options.volumes)) {
                ...
                let options = {
                    url: [config.cloud_api_url, 'v2', 'organizations', config.organization, 'backups', container_options.env_vars.CSC_BACKUP_ID, 'volumes', container_options.id].join('/'),
                    method: 'GET',
                    headers: {
                        Authorization: ['Bearer', config.api_key].join(' ')
                    }
        ...
        core.cluster.legiond.on('containership-cloud.backup', function(message) {
            let options = {
                url: [config.cloud_api_url, 'v2', 'organizations', config.organization, 'backups', message.data.backup_id, 'volumes', message.data.container_id].join('/'),
                method: 'POST',
                headers: {
                    Authorization: ['Bearer', config.api_key].join(' ')
                }
