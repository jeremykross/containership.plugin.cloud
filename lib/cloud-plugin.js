...
class ContainershipCloudPlugin extends ContainershipPlugin {
    ...
    serveRPC() {
        RPC.serve(4481, ContainershipCloudPlugin.restoreBackup);
    }

    apiRequest(verb, endpoint, data, cb) {
        console.log("Making request in cloud: " + verb + " @ " + endpoint + " with " + JSON.stringify(data));
        ...
        if(api_key && organization) {
            ...
            const logError = (err) => {
                console.log("ERROR in cloud plugin: " + verb + " // " + constants.environment.CLOUD_API_BASE_URL + " / " + endpoint + " -> " + JSON.stringify(err.message));
            };
    ...
    setDistributedValueIfNeeded(k, v) {
        if(!this.isCached(k, v)) {
            console.log("Updating value & cache for: " + k + " and value " + v);
    ...
    getApiRoutes(host) {
        ...
        return new ApiBuilder()
            .get("/cluster/backup", (req, res) => {
                ...
                if(host.isLeader()) {
                    console.log("Received a backup request from the server: " + JSON.stringify(req.body));
                    this.backupRequestLeader(host, req, res);
                } else {
                    console.log("Received a backup request on the follower: " + JSON.stringify(req.body));
