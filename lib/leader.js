...
module.exports = {
    ...
    initialize: function(core, config) {
        ...
        function register_cluster(callback) {
            ...
            const options = {
                url: `${config.cloud_api_url}/v2/organizations/${config.organization}/clusters/${core.cluster_id}`,
                method: 'POST',
                timeout: 5000,
                headers: {
        ...
        function sync_loadbalancers(callback) {
            ...
            if (attributes.praetor.leader) {
                const options = {
                    url: `${config.cloud_api_url}/v2/organizations/${config.organization}/clusters/${core.cluster_id}/loadbalancers`,
                    method: 'GET',
                    timeout: 5000,
                    headers: {
                        Authorization: `Bearer ${config.api_key}`
